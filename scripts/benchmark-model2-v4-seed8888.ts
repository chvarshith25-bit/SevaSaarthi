/**
 * Seva Saarthi Model 2 V4 - 1,000-Query Clean Seed=8888 Comparative Benchmark
 * 
 * Compares:
 * 1. Model A: V1 Deterministic Baseline
 * 2. Model B: V3.1 Calibrated Demographic Evidence Baseline
 * 3. Model C: REAL Transformer-Only Baseline (Pretrained intfloat/multilingual-e5-base)
 * 4. Model D: V4 Hybrid Transformer Model
 * 
 ...
 * - Person-level ground truth evaluation post-scoring.
 */
...
export async function generateCleanSeed8888Benchmark(): Promise<BenchmarkQuery[]> {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    queries.push({
      id: `CLEAN-BENCH-8888-${String(queryId++).padStart(4, '0')}`,
      category: cat,
      language: lang,
...
async function main() {
  console.log('========================================================================');
  console.log('   SEVA SAARTHI: MODEL 2 V4 SEED=8888 CLEAN COMPARATIVE BENCHMARK        ');
  console.log('========================================================================\n');
  ...
  const benchmarkQueries = await generateCleanSeed8888Benchmark();
  console.log(`Generated ${benchmarkQueries.length} clean benchmark queries (Seed: 8888).`);
  ...
  const runEvaluation = async (modelType: 'V1' | 'V3.1' | 'REAL_TRANSFORMER_ONLY' | 'V4') => {
    ...
    for (const item of benchmarkQueries) {
      ...
      // Pass strictly isolated query to model scoring path (no ground truth in query object)
      ...
      if (modelType === 'V1') {
        ...
        // Pure Real Transformer Ranking: Database retrieval + Cosine Similarity of 768-D embeddings
        const rawCandidates = await (v4Engine as any).retrieveCandidateRows(sanitizedQuery, 25);
        if (rawCandidates.length > 0) {
          ...
          // Sort purely descending by real cosine similarity
          ...
          const candidateMatches = scoredCandidates.map((c) => ({
            ...
            isCollisionWarning: false, // Transformer alone lacks demographic collision checks!
        ...
        const res = await v4Engine.resolve(sanitizedQuery);
        const top = res.bestMatch;
        if (top) {
          bestCitId = top.citizenId;
          top3CitIds = res.candidates.slice(0, 3).map((c) => c.citizenId || '');
      ...
      // POST-SCORING: Score against ground truth ONLY now
  ...
  console.log('\n========================================================================');
  console.log('                 CLEAN SEED=8888 BENCHMARK RESULTS                      ');
  console.log('========================================================================');
  console.table([resV1, resV3, resTrans, resV4]);

  const outputData = {
    benchmark_version: 'v4.0.0-seed8888-real-transformer',
  ...
  fs.writeFileSync(
    path.resolve(process.cwd(), 'data/ai/entity-resolution/v4/clean_seed8888_summary.json'),
    JSON.stringify(outputData, null, 2)
  );
  console.log('\nBenchmark summary recorded to data/ai/entity-resolution/v4/clean_seed8888_summary.json');
}

main().catch((err) => {
  console.error('[FATAL] Seed 8888 benchmark error:', err);
  process.exit(1);
});
