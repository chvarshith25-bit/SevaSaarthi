    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-5 sm:p-6 shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="w-full md:w-72 shrink-0">
            <label className="block text-[11px] font-bold text-slate-500 mb-1">
              {t("select_scheme_service", "Select Scheme / Service:")}
            </label>
                ...
                {services.map((s) => {
                  ...
                  return (
                    <option key={s.id} value={s.id}>
                      {s.name} ({r.percent}% {t("ready", "Ready")})
                    </option>
                  );
                })}
              </select>
        ...
        <div className="pt-2 border-t border-slate-100">
          <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2.5 flex items-center justify-between">
            <span>{t("available_schemes", "Available Government Schemes & Certificates")} ({services.length})</span>
            <span className="text-[10px] text-indigo-600 font-semibold lowercase">{t("click_to_check", "click any card to check eligibility")}</span>
          </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100 flex items-center gap-1">
                ...
                {t("verified_scheme", "Verified Scheme")}
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2.5 mt-4">
              ...
              >
                <span>{t("official_portal", "Official Portal")} ({service.official_domain})</span>
              ...
              >
                ...
                <span>{t("autofill_assistant", "Autofill Assistant")}</span>
              </button>
              ...
              >
                ...
                <span>{showProcessRoadmap ? t("hide_process_guide", "Hide Process Guide") : t("process_guidelines", "Application Process & Guidelines")}</span>
              </button>
              ...
              >
                ...
                <span>{t("copy_summary", "Copy Summary")}</span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-slate-50 to-indigo-50/40 border border-slate-100 rounded-2xl p-5 min-w-[250px] text-center shrink-0">
            <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
              {t("readiness_score", "Readiness Score")}
            </div>
            <div className="text-3xl font-black text-slate-900 tracking-tight">
              {percentageComplete}%
            </div>
            <div className="text-xs font-medium text-slate-600 mt-0.5 mb-3">
              {satisfiedCount + manuallyResolvedCount} of {totalRequirements} {t("requirements_satisfied", "requirements satisfied")}
            </div>
            ...
            <div className="flex items-center justify-between text-[10px] font-semibold text-slate-500">
              <span className="text-emerald-700 font-bold">{satisfiedCount} {t("satisfied", "Satisfied")}</span>
              <span className="text-amber-700 font-bold">{missingCount} {t("missing_label", "Missing")}</span>
              <span className="text-blue-700 font-bold">{manuallyResolvedCount} {t("resolved", "Resolved")}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-3xl border border-amber-200/80 p-6 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-900">{t("missing_requirements_title", "Missing Requirements (What You Still Need)")}</h2>
                <p className="text-[11px] text-slate-500">{t("missing_requirements_desc", `Upload or confirm these ${missingItems.length} items to achieve 100% readiness for this scheme`)}</p>
              </div>
            </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                      ...
                      >
                        <UploadCloud className="w-3.5 h-3.5" />
                        <span>{t("upload_document", "Upload")}</span>
                      </button>
                      ...
                      >
                        ...
                        <span>{t("how_to_get", "How to get")}</span>
                      ...
                      >
                        ...
                        <span>{t("mark_resolved", "Mark Resolved")}</span>
                      </button>
                    </div>
                  </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-xs">
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900">{t("satisfied_requirements_title", "Satisfied Requirements (What You Already Have)")}</h2>
            <p className="text-[11px] text-slate-500">{t("satisfied_requirements_desc", "Verified and ready from your profile fields and vault documents")}</p>
          </div>
        </div>

        {satisfiedItems.length === 0 ? (
          <div className="p-6 bg-slate-50 rounded-2xl text-center text-slate-500 text-xs">
            {t("no_satisfied_requirements", "No satisfied requirements yet for this scheme. Upload your documents or complete your profile to satisfy them automatically.")}
          </div>
        ) : (
